...
const file_name = 'all_doctorstimetables.xlsx';

const sheet_to_class_mapper = (sheet_name, db_object) => {
	if (sheet_name === 'all_doctorstimetables'){

		return new DoctorsTimetable(db_object)
	
	} else if (sheet_name === 'doctorsappointments'){

	return new DoctorsAppointment(db_object)
...
const save_parent_and_children_in_db = (parent_children_rows_dict, sheet_to_class_dict) =>{
	...
	for (let i = 0; i < row_details_list.length; i++) {
		...
		const doctorstimetable = sheet_to_class_mapper(parent_sheet, {...parent_db_object_dict, _id: new mongoose.Types.ObjectId()})

		doctorstimetable.save(function (err, doctorstimetable) {
			...
			if (related_children.length > 0){
				for (let k = 0; k < related_children.length; k++) {
					...
					for (let l = 0; l < child_rows.length; l++) {
						...
						const related_child = sheet_to_class_mapper(child_sheet, {...child_db_object_dict, doctorstimetable: doctorstimetable._id})
						...
						if (child_sheet === 'doctorsappointments'){

							doctorstimetable.doctorsappointments.push(related_child._id)
						}
